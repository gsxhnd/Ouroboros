...
onMounted(() => {
  ...
  if (contentWidth.value === 0) {
    contentWidth.value = Number.parseInt(
      window.getComputedStyle(content.value).width
    );
  }
});
...
const itemRenderList = computed<SpaceOption[]>(() => {
  if (content.value == undefined || content.value.parentElement == null) {
    return [];
  }
  ...
  const [topPreloadScreenCount, bottomPreloadScreenCount] =
    props.preloadScreenCount;
  // 避免多次访问
